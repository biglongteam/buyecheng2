...
public class ChatActivity1 extends AChatActivity implements OnClickListener,
		...
		btn_speak.setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				...
				case MotionEvent.ACTION_UP:
					...
					if ("".equals(mFileName)) {
						Toast.makeText(ChatActivity1.this, "不能为空",
	...
	private void init() {
		titleBack = (TextView) findViewById(R.id.custom_actionbar_back);
		...
		adapter = new MessageListAdapter(ChatActivity1.this, getMessages(),
				listView);
		// 头
				...
				if (message.getContent().contains(CommonUtils.PIC_SIGN)) {
					...
					imageMsg.setOnClickListener(new OnClickListener() {
						public void onClick(View v) {
							...
							intent.setClass(ChatActivity1.this, ImageShower.class);
							startActivity(intent);
						}
					});
	...
	public void onClick(View v) {
		...
		if(id== R.id.edit_user_comment){
			...
			String message = messageInput.getText().toString();
			if ("".equals(message)) {
				Toast.makeText(ChatActivity1.this, "不能为空", Toast.LENGTH_SHORT)
						.show();
			} else {
			...
			public void run() {
				...
				if (mToast == null) {
					mToast = Toast.makeText(
							ChatActivity1.this.getApplicationContext(), resId,
							Toast.LENGTH_LONG);
				} else {
